...
async function saveEmp() {
    ...
    const employeeData = {
        ...
        panNumber:
            panNumber.value.trim().toUpperCase(),
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 401) {
                ...
                serverError.value =
                    data.message ||
                    'You are not authorized to register an employee.'
                ...
                serverError.value =
                    data.message ||
                    data.error ||
                    `Employee registration failed (${response.status})`
        ...
        serverError.value =
            'Unable to connect to the server. Please check the backend URL.'
